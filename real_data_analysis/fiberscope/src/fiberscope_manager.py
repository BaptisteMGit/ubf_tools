...
import real_data_analysis.fiberscope.src.params as p
...
class FiberscopeManager:
    ...
    def set_stft_params(self, ts):
        ...
        self.cm = CovManager(nperseg=nperseg, noverlap=noverlap)
    ...
    def get_rtf(self, xr_data, Rv_global=None):
        ...
        idx_hydro_ref = np.argmin(np.abs(xr_data.h_index.values - self.h_index_ref))
    ...
    def get_signal_presence_mask(self, x, fs, nperseg, noverlap):
        ...
        for ircv in range(stft_x.shape[0]):
            ...
            idx_peaks = sp.find_peaks(energy, height=0.1 * np.max(energy))[0]
