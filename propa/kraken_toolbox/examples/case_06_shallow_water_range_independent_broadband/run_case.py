...
# -*-coding:utf-8 -*-
"""
Case 6 -- Shallow-water waveguide, range-independent, broadband,
realistic sound-speed profile, semi-infinite isovelocity sediment.

Same environment as Case 5 (illustrative summer shallow-water SSP,
sandy isovelocity sediment half-space), run over several frequencies.

NOTE: the SSP values below are a representative, illustrative synthetic
profile, NOT measured data -- replace them with your own
...
ENV_FILENAME = "case_06_shallow_ri_broadband"
RUN_KRAKEN = os.environ.get("KRAKEN_EXAMPLES_RUN_KRAKEN", "0") == "1"

# ----------------------------------------------------------------------
# 1. Environment: same shallow-water SSP as Case 5, several frequencies
# ----------------------------------------------------------------------
WATER_DEPTH = 100.0  # m
FREQS = np.array([100.0, 200.0, 300.0, 400.0, 500.0])  # Hz
SRC_DEPTH = 20.0  # m
...
env = KrakenEnv(
    title="Case 6 - Shallow water, realistic SSP (range-independent, broadband)",
    env_root=HERE,
