...
def testcase1_0(testcase_varin):
    """
...
##########################################################################################
# Test case 2 : Anisotropic environment with RHUM RUM environment
##########################################################################################
...
##########################################################################################
# Test case 3 : Anisotropic environment with shallow water environment
##########################################################################################


def testcase3_common(freq, bathy, title, testcase_name="testcase3", rcv_r_max=50):
    # Create environment directory
    env_dir = os.path.join(TC_WORKING_DIR, testcase_name)
    if not os.path.exists(env_dir):
        os.makedirs(env_dir)
    ...
    data_dir = r"C:\Users\baptiste.menetrier\Desktop\devPy\phd\data\ssp\mmdpm"
    fpath = os.path.join(
        data_dir, "shallow_water", f"fevrier_lon_-5.87_-2.87_lat_51.02_54.02_ssp.mat"
    )
    ssp_mat = sio.loadmat(fpath)
    z_ssp = ssp_mat["ssp"]["z"][0, 0].flatten()
    cp_ssp = ssp_mat["ssp"]["c"][0, 0].flatten()
...
def testcase3_1(
    ...
    """
    Test case 3.1: Shallow water environment with real bathy profile extracted using MMDPM app. 1 layer bottom and realistic sound speed profile
    """
    ...
    name = "testcase3_1"
    title = "Test case 3.1: Anisotropic environment with real bathy profile extracted using MMDPM app. 1 layer bottom and realistic sound speed profile"
...
if __name__ == "__main__":
    ...
    #     tc_varin["azimuth"] = az
    #     env, flp = testcase3_1(tc_varin, plot_bathy=True)
